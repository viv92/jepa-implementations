...
def deterministic_sampling_heun_cfg(online_net, pred_net, use_cnoise, img_shape, rho, ts, class_label, default_label, x_inpaint, sigma_data, N, cfg_scale):
...
# NOTE that step range [0 ... N-1, N] = time range [T ... t_eps, 0] = noise range [sigma_max ... sigma_min, 0]
def stochastic_sampling_heun_cfg(online_net, pred_net, use_cnoise, img_shape, rho, ts, gammas, S_noise, class_label, default_label, x_inpaint, sigma_data, N, cfg_scale, start_time, device):
...
if __name__ == '__main__':
    ...
    model_save_freq = int(total_train_steps / 20)
    ...
    hyperparam_dict['latentDim'] = latent_dim
    hyperparam_dict['latentSeq'] = latent_seq_len
    ...
    save_folder = './results/jepa_edm_mnist' + hyperparam_str
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)

    # save ckpt path
    online_net_ckpt_path = './ckpts/jepa_edm_mnist_onlineNet' + hyperparam_str + '.pt'
    pred_net_ckpt_path = './ckpts/jepa_edm_mnist_predNet' + hyperparam_str + '.pt'
    target_net_ckpt_path = './ckpts/jepa_edm_mnist_targetNet' + hyperparam_str + '.pt'
    ...
    online_net = init_jepa_dit(max_seq_len_dit, seq_len, latent_seq_len, d_model_dit, patch_dim, latent_dim, condition_dim, d_k_dit, d_v_dit, n_heads_dit, n_layers_dit, d_ff_dit, dropout, device).to(device)
    
    pred_net = init_jepa_dit(max_seq_len_dit, seq_len, latent_seq_len, d_model_pred_dit, latent_dim, latent_dim, condition_dim, d_k_pred_dit, d_v_pred_dit, n_heads_pred_dit, n_layers_pred_dit, d_ff_pred_dit, dropout, device).to(device)
    ...
    # optimizer 
    optimizer_online_net = torch.optim.AdamW(params=online_net.parameters(), lr=lr)
    optimizer_pred_net = torch.optim.AdamW(params=pred_net.parameters(), lr=lr)
    ...
    while epoch < num_epochs:
        ...
        for imgs, labels in pbar2:
            ...
            if use_cnoise:
                ...
                t_n_input = t_n 
            online_latent = online_net(x_n_input, t_n_input, enc_out)
            ...
            train_step += 1
            pbar2.update(1)
            pbar2.set_description('loss:{:.3f}'.format(results_train_loss[-1]))
            ...
            if train_step % sample_freq == 0:
                ...
                with torch.no_grad():
                    ...
                    if sampling_strategy == 'deterministic':
                        sampled_latent, online_latent, pred_latent = deterministic_sampling_heun_cfg(online_net, pred_net, use_cnoise, sample_shape, rho, ts, sample_caption_emb, default_caption_emb, sample_xinpaint_emb, sigma_data, N_final, cfg_scale)
                    else:
                        sampled_latent, online_latent, pred_latent = stochastic_sampling_heun_cfg(online_net, pred_net, use_cnoise, sample_shape, rho, ts, gammas, S_noise, sample_caption_emb, default_caption_emb, sample_xinpaint_emb, sigma_data, N_final, cfg_scale, start_time, device)
