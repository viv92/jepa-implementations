...
class DiT(nn.Module):
    def __init__(self, embedder, decoder, max_seq_len_dpct, x_seq_len, d_model, x_dim, device):
        ...
        self.x_seq_len = x_seq_len 
        self.d_model = d_model
        self.final_proj = nn.Linear(d_model, x_dim, bias=False) # denoised output 
        self.device = device
...
def init_dit(max_seq_len_dit, x_seq_len, d_model, x_dim, condition_dim, d_k, d_v, n_heads, n_layers, d_ff, dropout, device):
    ...
    dit_embedder = DiT_Embedder(max_seq_len_dit, x_seq_len, d_model, x_dim, dropout, positional_encoder_fixed, condition_dim, device)
    model = DiT(dit_embedder, decoder, max_seq_len_dit, x_seq_len, d_model, x_dim, device)
